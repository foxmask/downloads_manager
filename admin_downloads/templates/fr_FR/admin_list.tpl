...
<table class="records-list">
    ...
    <tr class="{cycle array('even','odd')}">
        ...
        <td class="name"><a href="{jurl 'admin_downloads~mgr:manage',array('id'=>$download->id)}" title="Editer ce téléchargement">{$download->dl_name}</a></td>
        <td>{$download->dl_date|jdatetime:'db_date':'lang_datetime'}</td>
        <td>{$download->dl_count}</td>
        <td>{if $download->dl_enable == 1}<a href="{jurl 'admin_downloads~mgr:dl_disable',array('id'=>$download->id)}" title="ce téléchargement est actif, cliquer pour le désactiver"><img src="{$j_themepath}img/on.png" alt="actif"/></a>{else}<a href="{jurl 'admin_downloads~mgr:dl_enable',array('id'=>$download->id)}" title="ce téléchargement est inactif, cliquer pour l'activer"><img src="{$j_themepath}img/off.png" alt="inactif"/></a>{/if}</td>
        <td>
        <a href="{jurl 'admin_downloads~mgr:delete',array('id'=>$download->id)}" title="Supprimer ce téléchargement ?"><img src="{$j_themepath}img/delete.png" alt="supprimer" onclick="return confirm('Etes vous sûr de vouloir supprimer ce téléchargement ?')"/></a>
